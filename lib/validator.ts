...
export const StockMovementInputSchema = z.object({
  ...
  quantity: z.number(),
  previousStock: z.number().int().nonnegative('Previous stock must be non-negative'),
...
export const InventoryFiltersSchema = z.object({
  ...
  sort: z.enum(['latest', 'oldest', 'name-asc', 'name-desc', 'stock-low', 'stock-high']).default('latest'),
})
...
const UserRole = z.enum(['admin', 'manager', 'seller', 'user'], {
  errorMap: () => ({ message: 'Invalid role. Must be admin, manager, seller, or user' })
})
...
// Admin user creation schema (simplified - no address/payment required)
